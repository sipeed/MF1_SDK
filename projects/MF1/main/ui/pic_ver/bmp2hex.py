...
def main():
    ...
    parser.add_argument(
        "-k", "--k210", help="Outputs as K210 AI RGB format", action="store_true")
    parser.add_argument(
        "-kbin", "--k210bin", help="Outputs as K210 RGB565 (pix swap) format", action="store_true")
    parser.add_argument(
        "-i", "--invert", help="Inverts bitmap pixels", action="store_true")
    parser.add_argument(
        "-w", "--width", help="Output table width in hex bytes [default: 16]", type=int)
    parser.add_argument(
        "-b", "--bytes", help="Byte width of BMP sizes: 0=auto, 1, or 2 (big endian) [default: 0]", type=int)
    parser.add_argument("-n", "--named", help="Uses named structure (" +
    ...
    parser.add_argument(
        "-x", "--xbm", help="Uses XBM bit order (low order bit is first pixel of byte)", action="store_true")
    parser.add_argument(
        "-v", "--version", help="Returns the current bmp2hex version", action="store_true")
    parser.add_argument(
        "-be", "--bigendian", help="save bigendian or little endian", action="store_true")
    ...
    if args.bigendian:
        print("convert bin, flash use quad to read")
        save_be=args.bigendian
    ...
    for f in args.infile:
        if f == '-':
            sys.exit()
        bmp2hex(f.name, tablewidth, sizebytes, invert,
                raw, named, double, xbm, k210, k210bin, save_be)
...
def bmp2hex(infile, tablewidth, sizebytes, invert, raw, named, double, xbm, k210, k210bin, save_be):
    ...
    if k210:
        ...
        if (raw):
            ...
            if (not (sizebytes % 2)):
                ...
                print("{0:#04X}".format(pixelWidth & 0xFF) + ", " +
            ...
            print('  unsigned int   height;')
            print('  unsigned int   bitDepth;')
            print('  uint8_t        pixel_data[{0}];'.format(
                byteWidth * pixelHeight))
            print('} ' + tablename + ' = {')
            print('{0}, {1}, {2}, {{'.format(
        ...
        if k210bin:
            ...
            for i in range(pixelHeight):
                for j in range(byteWidth//6):
                    ...
                    pix0 = (pix0_b >> 3) | ((pix0_g >> 2) << 5) | (
                        (pix0_r >> 3) << (5+6))
                    pix1 = (pix1_b >> 3) | ((pix1_g >> 2) << 5) | (
                        (pix1_r >> 3) << (5+6))
