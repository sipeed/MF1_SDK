...
static void irq_RS_Sync(void)
{
    ...
    if(PageCount >= LCD_SIPEED_FRAME_END_LINE)
    {
        dis_flag = 0;
    }
}
...
static void lcd_sipeed_send_cmd(uint8_t CMDData)
{
    ...
    spi_send_data_normal_dma(DMAC_CHANNEL0, LCD_SIPEED_SPI_DEV, LCD_SIPEED_SPI_SS, (uint8_t *)(&CMDData), 1, SPI_TRANS_CHAR);
}

static void lcd_sipeed_send_dat(uint8_t *DataBuf, uint32_t Length)
{
    spi_send_data_normal_dma(DMAC_CHANNEL0, LCD_SIPEED_SPI_DEV, LCD_SIPEED_SPI_SS, (uint8_t *)DataBuf, Length / 4, SPI_TRANS_INT);
}
...
static int timer_callback(void *ctx)
{
    ...
    printk("timer_callback %ld us\r\n",tim - int_tim);
    int_tim = tim;
    dis_flag = 1;
    lcd_sipeed_display(lcd_image, 0);
    // printk("\t timer callback out\r\n");
}
...
void lcd_init(void)
{
    ...
    timer_init(1);
    timer_set_interval(1, 1, 70 * 1000 * 1000); //150ms
    timer_irq_register(1, 1, 0, 3, timer_callback, NULL);
...
void copy_image_cma_to_lcd(uint8_t *cam_img, uint8_t *lcd_img)
{
    ...
    for(y = 0; y < LCD_H; y++)
    {
        memcpy(lcd_img + y * LCD_W * 2, cam_img + (y * IMG_W + x) * 2, IMG_W * 2);
    }
    return;
}
...
void lcd_sipeed_display(uint8_t *img_buf, uint8_t block)
{
    ...
    gpiohs_set_irq(LCD_DCX_HS_NUM, 2, irq_RS_Sync);
