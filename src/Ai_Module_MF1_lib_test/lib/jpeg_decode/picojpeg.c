...
static int16 cbG_tab[256];
static int16 cbB_tab[256];

static int16 crR_tab[256];
static int16 crG_tab[256];

static uint8 tab_flag = 0;

static void jpeg_init_tab(void)
{
   uint8 i;

   for(i = 0; i < 0xff; i++)
   {
      cbG_tab[i] = ((i * 88U) >> 8U) - 44U;
      cbB_tab[i] = (i + ((i * 198U) >> 8U)) - 227U;
      crR_tab[i] = (i + ((i * 103U) >> 8U)) - 179;
      crG_tab[i] = ((i * 183U) >> 8U) - 91;
   }
   tab_flag = 1;
}
...
static void upsampleCb(uint8 srcOfs, uint8 dstOfs)
{
   ...
   for (y = 0; y < 4; y++)
   {
      for (x = 0; x < 4; x++)
      {
         ...
         cbB = cbB_tab[cb];// : (cb + ((cb * 198U) >> 8U)) - 227U;
...
static void upsampleCr(uint8 srcOfs, uint8 dstOfs)
{
   ...
   for (y = 0; y < 4; y++)
   {
      for (x = 0; x < 4; x++)
      {
         ...
         crR = crR_tab[cr];// : (cr + ((cr * 103U) >> 8U)) - 179;
   ...
   switch (gScanType)
   {
      ...
      case PJPG_YH1V1:
      {
         ...
         switch (mcuBlock)
         {
            ...
            case 1:
            {
               cbG = cbG_tab[c];// : ((c * 88U) >> 8U) - 44U;
               gMCUBufG[0] = subAndClamp(gMCUBufG[0], cbG);

               cbB =  cbB_tab[c];// : (c + ((c * 198U) >> 8U)) - 227U;
               gMCUBufB[0] = addAndClamp(gMCUBufB[0], cbB);
               break;
            }
            case 2:
            {
               crR = crR_tab[c];// : (c + ((c * 103U) >> 8U)) - 179;
               gMCUBufR[0] = addAndClamp(gMCUBufR[0], crR);

               crG = crG_tab[c];// : ((c * 183U) >> 8U) - 91;
