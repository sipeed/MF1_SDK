...
bool image_get_mask_pixel(ui_image_t *ptr, int x, int y)
{
...
static void point_fill(ui_image_t *img, int cx, int cy, int r0, int r1, int c)
{
    for (int y = r0; y <= r1; y++)
    {
        for (int x = r0; x <= r1; x++)
...
static void xLine(ui_image_t *img, int x1, int x2, int y, int c)
{
    while (x1 <= x2)
        imlib_set_pixel(img, x1++, y, c);
}

static void yLine(ui_image_t *img, int x, int y1, int y2, int c)
{
    while (y1 <= y2)
...
static void scratch_draw_pixel(ui_image_t *img, int x0, int y0, int dx, int dy, float shear_dx, float shear_dy, int r0, int r1, int c)
{
    point_fill(img, x0 + dx, y0 + dy + fast_floorf((dx * shear_dy) / shear_dx), r0, r1, c);
}

// https://scratch.mit.edu/projects/50039326/
static void scratch_draw_line(ui_image_t *img, int x0, int y0, int dx, int dy0, int dy1, float shear_dx, float shear_dy, int c)
{
...
// https://scratch.mit.edu/projects/50039326/
static void scratch_draw_sheared_ellipse(ui_image_t *img, int x0, int y0, int width, int height, bool filled, float shear_dx, float shear_dy, int c, int thickness)
{
    int thickness0 = (thickness - 0) / 2;
    int thickness1 = (thickness - 1) / 2;
...
// https://scratch.mit.edu/projects/50039326/
static void scratch_draw_rotated_ellipse(ui_image_t *img, int x, int y, int x_axis, int y_axis, int rotation, bool filled, int c, int thickness)
{
...
void imlib_draw_string(ui_image_t *img, int x_off, int y_off, const char *str, int c, float scale, int x_spacing, int y_spacing, bool mono_space)
{
    ...
    for (char ch, last = '\0'; (ch = *str); str++, last = ch)
    {
        ...
        else
        {
            ...
            if (!exit)
                x_off += fast_roundf(scale * 3); // space char
        }
    }
}

static int safe_map_pixel(ui_image_t *dst, ui_image_t *src, int pixel)
{
    switch (dst->bpp)
    {
    case IMAGE_BPP_RGB565:
    ...
    for (int y = 0, yy = fast_roundf(other->h * y_scale); y < yy; y++)
    {
        ...
        for (int x = 0, xx = fast_roundf(other->w * x_scale); x < xx; x++)
        {
            ...
            if ((!mask) || image_get_mask_pixel(mask, other_x, other_y))
            {
                ...
                imlib_set_pixel(img, x_off + x, y_off + y, (alpha == 1) ? pixel : (pixel * alpha) + (imlib_get_pixel(img, x_off + x, y_off + y) * beta));
            }
        }
    }
}
