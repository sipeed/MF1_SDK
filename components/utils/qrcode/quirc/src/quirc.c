...
{
    uint64_t tmp_size = 50 * 1024;
    // ptr->data = (char *) fb_alloc_all(&tmp_size);
    ptr->data = (char *)malloc(tmp_size);
    ptr->data_len = data_len;
                            ...
                            const struct neighbour_list *vlist)
{
    ...
    for (j = 0; j < hlist->count; j++)
        for (k = 0; k < vlist->count; k++)
        {
            ...
            float score = fabsf(1.0 - hn->distance / vn->distance);
...
struct quirc *quirc_new(void)
{
    struct quirc *q = (struct quirc *)malloc(sizeof(*q)); //TODO: free
    if (!q)
        return NULL;
    memset(q, 0, sizeof(*q));
    return q;
...
int quirc_resize(struct quirc *q, int w, int h)
{
    ...
    uint8_t *new_image = (uint8_t *)malloc(w * h); //TODO: free
    ...
    { //should gray, 1==1
        ...
        if (q->pixels)
            free(q->pixels);
        quirc_pixel_t *new_pixels = (quirc_pixel_t *)malloc(new_size); //TODO: free
        if (!new_pixels)
        {
            if (q->image)
            {
...
uint8_t find_qrcodes(qrcode_result_t *out, qrcode_image_t *img, qrcode_scan_t *scan, uint8_t convert)
{
    ...
    qrcode_convert_to_gray(img, grayscale_image);
    ...
    for (int i = 0, j = quirc_count(controller); i < j; i++)
    {
        ...
        {
            if (data.payload_len != 0 && data.payload_len < QUIRC_MAX_PAYLOAD)
            {
                out->payload_len = data.payload_len;
                memcpy(out->payload, data.payload, data.payload_len);
                out->payload[data.payload_len] = 0;

                out->version = data.version;
                out->ecc_level = data.ecc_level;
                out->mask = data.mask;
