...
static uint8_t spi_rw_high_spd(uint8_t data)
{
    ...
    {
        GPIOHS_OUT_LOWX(CONFIG_TF_SPI_GPIOHS_NUM_SCLK);
        if (data & 0x80)
        {
            GPIOHS_OUT_HIGH(CONFIG_TF_SPI_GPIOHS_NUM_MOSI);
        }
        else
        {
            GPIOHS_OUT_LOWX(CONFIG_TF_SPI_GPIOHS_NUM_MOSI);
        }
        data <<= 1;
        temp <<= 1;
        if (GET_GPIOHS_VALX(CONFIG_TF_SPI_GPIOHS_NUM_MISO))
        {
            temp++;
        }
        GPIOHS_OUT_HIGH(CONFIG_TF_SPI_GPIOHS_NUM_SCLK);
        asm volatile("nop");
        asm volatile("nop");
    }
    return temp;
...
{
    //cs
    fpioa_set_function(CONFIG_TF_SPI_PIN_CS, FUNC_GPIOHS0 + CONFIG_TF_SPI_GPIOHS_NUM_CS);
    gpiohs_set_drive_mode(CONFIG_TF_SPI_GPIOHS_NUM_CS, GPIO_DM_OUTPUT);
    gpiohs_set_pin(CONFIG_TF_SPI_GPIOHS_NUM_CS, 1);
    //clk
    fpioa_set_function(CONFIG_TF_SPI_PIN_SCLK, FUNC_GPIOHS0 + CONFIG_TF_SPI_GPIOHS_NUM_SCLK);
    gpiohs_set_drive_mode(CONFIG_TF_SPI_GPIOHS_NUM_SCLK, GPIO_DM_OUTPUT);
    gpiohs_set_pin(CONFIG_TF_SPI_GPIOHS_NUM_SCLK, 0);
    //mosi
    fpioa_set_function(CONFIG_TF_SPI_PIN_MOSI, FUNC_GPIOHS0 + CONFIG_TF_SPI_GPIOHS_NUM_MOSI);
    gpiohs_set_drive_mode(CONFIG_TF_SPI_GPIOHS_NUM_MOSI, GPIO_DM_OUTPUT);
    gpiohs_set_pin(CONFIG_TF_SPI_GPIOHS_NUM_MOSI, 0);
    //miso
    fpioa_set_function(CONFIG_TF_SPI_PIN_MISO, FUNC_GPIOHS0 + CONFIG_TF_SPI_GPIOHS_NUM_MISO);
    gpiohs_set_drive_mode(CONFIG_TF_SPI_GPIOHS_NUM_MISO, GPIO_DM_INPUT_PULL_UP);
}
...
{
    GPIOHS_OUT_HIGH(CONFIG_TF_SPI_GPIOHS_NUM_CS);
...
{
    GPIOHS_OUT_LOWX(CONFIG_TF_SPI_GPIOHS_NUM_CS);
    if (SD_WaitReady() == 0)
        return 0; //等待成功
    SD_DisSelect();
    return 1; //等待失败
