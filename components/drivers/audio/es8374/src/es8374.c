...
int maix_read_reg(uint8_t addr, uint8_t reg, uint8_t *data)
{
    ...
    *data = recv_data;
    if (res != 0)
        printk("i2c read reg error!, recv_data: %d\n", recv_data);
    return res;
}
//static
int maix_i2c_init()
...
int es8374_set_mic_gain(es_mic_gain_t gain)
{
    ...
    {
        res = -1;
        printk("invalid microphone gain!\n");
    }
...
int es8374_ctrl_state(es8374_mode_t mode, es8374_ctrl_t ctrl_state)
{
    ...
    default:
        es_mode_t = ES_MODULE_DAC;
        printk("Codec mode not support, default is decode mode\r\n");
        break;
    }
    ...
    else
    {
        ...
        printk("start default is decode mode:%d\r\n", es_mode_t);
    }
    return res;
}
