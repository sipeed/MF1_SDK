...
{
    // I2S OUT BUZZER
    fpioa_set_function(CONFIG_PIN_NUM_PT8211_DAT, FUNC_I2S2_OUT_D1);
    fpioa_set_function(CONFIG_PIN_NUM_PT8211_CLK, FUNC_I2S2_SCLK);
    fpioa_set_function(CONFIG_PIN_NUM_PT8211_WS, FUNC_I2S2_WS);
    // PA
...
static int audio_speaker_timer_tx_irq(void *ctx)
{
    ...
    else if (speaker->play_state == PLAY_STATE_STOP)
    {
        speaker->ispaly = 0;
        speaker->current_frame = 0;
        gpio_set_pin(CONFIG_GPIO_NUM_PT8211_PA_EN, GPIO_PV_LOW); // ADD By xel
    }
    else
    {
        speaker->ispaly = 0;
...
void audio_speaker_play()
{
    ...
    gpio_set_pin(CONFIG_GPIO_NUM_PT8211_PA_EN, 1); //ADD By xel
}
...
{
    audio->config = audio_speaker_init;
    audio->play = pt8211_play;
    return 0;
}
